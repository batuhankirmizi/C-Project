...
void AI_penguin_move(int board_size_X, int board_size_Y, int* board[], int player){ //a move command will be given according to the game logic/st
 ...
 for(a=0;a<board_size_X;a++)
 {
     for(b=0;b<board_size_Y;b++)
     {
        ...
        if(board[a][b]==3 && a==i) // right 3 fishes
         ...
         {
             if(check_move(board_size_X,board_size_Y,board,i,j,i-a,5)==1)
            {
            board[a][b]=board[i][j];
            board[i][j]=0;
            key=1;
         ...
         {
             if(check_move(board_size_X,board_size_Y,board,i,j,i-a,6)==1)
            {
            board[a][b]=board[i][j];
            board[i][j]=0;
            key=1;
