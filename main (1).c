...
// int number_of_players=0; ->moved inside int main to get rid of global variables
// int penguins_per_player=0;
/*
argc = argument count
argv = argument list, list of strings/char pointers
 ...
 srand(time(NULL));
        if (argc == 5 ){ // Format : -> player1.out phase=placement penguins=3 inputboard.txt outpuboard.txt
            ...
            */
            if(strcmp(argv[1],"phase=placement") == 0){ // if they are equal -> the second parameter phase=placement
                ...
                int player_number = 1;//argv[0][6], i use 1 to simplify and to get chance to compile our program in codeblocks //taking the player number as an int - e.g 1,2 etc. assuming program name player1, player2 etc. returns something stupid when you call it from codeblocks, need to command line compile
               ...
               // printf("% d", atoi(argv[2][9]));
                ...
                read_from_text_file_and_create_board(board_size_X,board_size_Y,board,argv[3]); //read the info from the text file, populate the board
                ...
                if(AI_check_if_all_placed(board_size_X,board_size_Y,board,penguin_count,player_number)==0){ //still there are penguins to place
                        AI_penguin_place(board_size_X,board_size_Y,board,player_number);
                ...
                write_to_text_file(board_size_X,board_size_Y,board,argv[4],argv[0]); //write to another text file after everything has been done
    ...
    // PHASE = MOVEMENT, argument count 4
            ...
            //how_many_players(board_size_X, board_size_Y, board);
                printf("Gotta do what you gotta do \n"); // execute accordingly
                int player_number = 1; //taking the player number as an int - e.g 1,2 etc.
                ...
                read_from_text_file_and_create_board(board_size_X,board_size_Y,board,argv[2]); //read the board configuration and store it in board

                AI_penguin_move(board_size_X,board_size_Y,board,player_number); // AI_function moves the penguin to some place - not implemented


                write_to_text_file(board_size_X,board_size_Y,board,argv[3],argv[0]); //writes the final board config
